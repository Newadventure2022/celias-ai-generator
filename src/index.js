...
function generateWords(event) {
  ...
  let wordArray = instructionsInput.value.split("");

  let emojiArray = wordArray.map((char) => {
    return char + " 😊";
  });

  let emojiString = emojiArray.join(" ");

  let prompt = `Generate a new encouraging message using the word ${emojiString} and sign the words with an emoji and <br/> <strong> '~ SheCodes AI'</strong>, but please do not put '' around SheCodes AI`;
